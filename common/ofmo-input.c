...
static int input_contrl( FILE *fp, char **token, const int ntok ) {
	...
	char *ansatz=(char*)malloc( sizeof(char) * MAXSTRLEN );
	char *threshold=(char*)malloc( sizeof(char) * MAXSTRLEN );
	char *version=(char*)malloc( sizeof(char) * MAXSTRLEN );
	vqescr[0] = '\0';
	desc[0] = '\0';
	ansatz[0] = '\0';
	threshold[0] = '\0';
    ...
    if ( fp == NULL ) {
	ofmo_data_put_vals("maxscf nprint itol icut method method_list method_dim_idx method_dim method_dim_n vqescr desc ansatz threshold version",
		                maxit, nprint,itol,icut,method,method_list,method_dim_idx,method_dim,method_dim_n,vqescr,desc,ansatz,threshold,version);
	return 0;
    }
	...
	for ( i=0; i<n; i++ ) {
		...
		else if ( strcmp( elems[0], "ansatz" ) == 0) {
			strcpy(ansatz, elems[1]);
		}
		else if ( strcmp( elems[0], "threshold" ) == 0) {
			strcpy(threshold, elems[1]);
		}
		else if ( strcmp( elems[0], "version" ) == 0) {
			strcpy(version, elems[1]);
		}
