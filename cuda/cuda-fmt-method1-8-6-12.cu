...
__device__ void gpu_fmt1_method1( const double t, const double coef, double fmt[] ) {
    ...
    if ( t >= 30 ) {
      ...
      double dhalf = delta * 0.5;
        ...
        t0 = delta * (double)it + dhalf;
        dt = t0 - t;
        f  = &FMT_m_table1[it*(1+6)];
#if 0
        dtk[1] = dt;
#pragma unroll
        for ( k=2; k<6; k++ ) dtk[k] = dtk[k-1]*(LDG(FMT_m_dinv[k])*dt);
        fmt[0] = LDM(f[0]) + dtk[1]*LDM(f[1]) + dtk[2]*LDM(f[2]) + dtk[3]*LDM(f[3])
                 + dtk[4]*LDM(f[4]) + dtk[5]*LDM(f[5]);
        fmt[0] *= coef;
        fmt[1] = LDM(f[1]) + dtk[1]*LDM(f[2]) + dtk[2]*LDM(f[3]) + dtk[3]*LDM(f[4])
                 + dtk[4]*LDM(f[5]) + dtk[5]*LDM(f[6]);
        fmt[1] *= coef;
#else
        double dtk1=dt;
        double fmt0, fmt1;
        fmt0 = LDM(f[0]) + dtk1*LDM(f[1]);
        fmt1 = LDM(f[1]) + dtk1*LDM(f[2]);
#pragma unroll
        for ( k=2; k<6; k++ ) {
          dtk1 = dtk1*(LDG(FMT_m_dinv[k])*dt);
          fmt0 += dtk1*LDM(f[k]);
          fmt1 += dtk1*LDM(f[k+1]);
        }
        fmt[0] = fmt0 * coef;
...
#pragma unroll
        ...
        fmt[2] = LDM(f[2]) + dtk[1]*LDM(f[3]) + dtk[2]*LDM(f[4]) + dtk[3]*LDM(f[5])
                 + dtk[4]*LDM(f[6]) + dtk[5]*LDM(f[7]);
        fmt[2] *= coef;
#else
        double dtk1=dt;
        double fmt0, fmt1, fmt2;
        fmt0 = LDM(f[0]) + dtk1*LDM(f[1]);
        fmt1 = LDM(f[1]) + dtk1*LDM(f[2]);
        fmt2 = LDM(f[2]) + dtk1*LDM(f[3]);
#pragma unroll
        for ( k=2; k<6; k++ ) {
          dtk1 = dtk1*(LDG(FMT_m_dinv[k])*dt);
          fmt0 += dtk1*LDM(f[k]);
