...
static void ofmo_hrr_calc_dpss(
...
static void ofmo_vrr_calc_dpss(
...
static void ofmo_vrr_cint_dpss( const double *ev, double *eh ) {
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_dpss(
                    ...
                    ev );
            ofmo_vrr_cint_dpss( ev, eh );
        }	// for (klps)
    }	// for (ijps)
    ofmo_hrr_calc_dpss( BA, DC, eh );
