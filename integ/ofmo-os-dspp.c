...
static void ofmo_hrr_calc_dspp(
...
static void ofmo_vrr_calc_dspp(
...
static void ofmo_vrr_cint_dspp( const double *ev, double *eh ) {
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_dspp(
                    ...
                    ev );
            ofmo_vrr_cint_dspp( ev, eh );
        }	// for (klps)
    }	// for (ijps)
    ofmo_hrr_calc_dspp( BA, DC, eh );
    ofmo_hrr_coef_dspp( eh, DINT );
}

int ofmo_twoint_os_dspp(
...
int ofmo_twoint_direct_os_dspp(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int shel_atm[], const int shel_ini[],
        const double atom_x[], const double atom_y[],
