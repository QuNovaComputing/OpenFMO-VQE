...
static void ofmo_hrr_clear_dsps( double *eh ) {
    int i;
    // (DS|PS)
    for ( i=0; i<(0+18); i++ ) eh[i] = 0.e0;
}

static void ofmo_hrr_coef_dsps(
...
static void ofmo_vrr_calc_dsps(
    ...
    // (ds|ps) m=[0,0]
    ev[25]=QC[0]*ev[13]+WQ[0]*ev[19]+2.e0*ze2*ev[7];
    ev[26]=QC[1]*ev[13]+WQ[1]*ev[19];
    ev[27]=QC[2]*ev[13]+WQ[2]*ev[19];
    ev[28]=QC[0]*ev[14]+WQ[0]*ev[20];
    ev[29]=QC[1]*ev[14]+WQ[1]*ev[20]+2.e0*ze2*ev[8];
    ev[30]=QC[2]*ev[14]+WQ[2]*ev[20];
    ev[31]=QC[0]*ev[15]+WQ[0]*ev[21];
    ev[32]=QC[1]*ev[15]+WQ[1]*ev[21];
    ev[33]=QC[2]*ev[15]+WQ[2]*ev[21]+2.e0*ze2*ev[9];
    ev[34]=QC[0]*ev[16]+WQ[0]*ev[22]+     ze2*ev[8];
    ev[35]=QC[1]*ev[16]+WQ[1]*ev[22]+     ze2*ev[7];
    ev[36]=QC[2]*ev[16]+WQ[2]*ev[22];
    ev[37]=QC[0]*ev[17]+WQ[0]*ev[23]+     ze2*ev[9];
    ev[38]=QC[1]*ev[17]+WQ[1]*ev[23];
    ev[39]=QC[2]*ev[17]+WQ[2]*ev[23]+     ze2*ev[7];
    ev[40]=QC[0]*ev[18]+WQ[0]*ev[24];
    ev[41]=QC[1]*ev[18]+WQ[1]*ev[24]+     ze2*ev[9];
    ev[42]=QC[2]*ev[18]+WQ[2]*ev[24]+     ze2*ev[8];
}

static void ofmo_vrr_cint_dsps( const double *ev, double *eh ) {
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_dsps(
                    ...
                    ev );
            ofmo_vrr_cint_dsps( ev, DINT );
        }	// for (klps)
    }	// for (ijps)
    ofmo_hrr_coef_dsps( DINT, DINT );
}

int ofmo_twoint_os_dsps(
...
int ofmo_twoint_direct_os_dsps(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int shel_atm[], const int shel_ini[],
        const double atom_x[], const double atom_y[],
