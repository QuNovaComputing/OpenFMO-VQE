...
static void ofmo_vrr_calc_dsss(
...
static void ofmo_vrr_cint_dsss( const double *ev, double *eh ) {
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_dsss(
                    ...
                    ev );
            ofmo_vrr_cint_dsss( ev, DINT );
        }	// for (klps)
    }	// for (ijps)
    ofmo_hrr_coef_dsss( DINT, DINT );
}
