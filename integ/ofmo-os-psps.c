...
static void ofmo_hrr_clear_psps( double *eh ) {
    int i;
    // (PS|PS)
    for ( i=0; i<(0+9); i++ ) eh[i] = 0.e0;
}

static void ofmo_vrr_calc_psps(
    ...
    // (ps|ps) m=[0,0]
    ev[ 9]=QC[0]*ev[3]+WQ[0]*ev[6]+ze2*ev[1];
    ev[10]=QC[1]*ev[3]+WQ[1]*ev[6];
    ev[11]=QC[2]*ev[3]+WQ[2]*ev[6];
    ev[12]=QC[0]*ev[4]+WQ[0]*ev[7];
    ev[13]=QC[1]*ev[4]+WQ[1]*ev[7]+ze2*ev[1];
    ev[14]=QC[2]*ev[4]+WQ[2]*ev[7];
    ev[15]=QC[0]*ev[5]+WQ[0]*ev[8];
    ev[16]=QC[1]*ev[5]+WQ[1]*ev[8];
    ev[17]=QC[2]*ev[5]+WQ[2]*ev[8]+ze2*ev[1];
}

static void ofmo_vrr_cint_psps( const double *ev, double *eh ) {
    int La=1, Lb=0, Lc=1, Ld=0;
    int i, ih, iv;
    // (PS|PS)
    for ( i=0, iv=9, ih=0; i<9; i++, iv++, ih++ ) eh[ih]+=ev[iv];
}

void ofmo_twoint_core_os_psps(
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_psps(
                    ...
                    ev );
            ofmo_vrr_cint_psps( ev, DINT );
        }	// for (klps)
    }	// for (ijps)
}

int ofmo_twoint_os_psps(
    ...
    max_nzeri = ebuf_max_nzeri - 3*1*3*1;
    nzeri4    = nzeri*4;
    if ( nzeri >= max_nzeri ) {
...
int ofmo_twoint_direct_os_psps(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int shel_atm[], const int shel_ini[],
        const double atom_x[], const double atom_y[],
