...
static void ofmo_vrr_calc_psss(
    ...
    // (ss|ss) m=0,1
    //fmt( &ev[0], 1, T, cssss );
    OFMO_FMT( &ev[0], 1, T, cssss );
    // (ps|ss) m=0,0
    ev[2]=PA[0]*ev[0]+WP[0]*ev[1];
    ev[3]=PA[1]*ev[0]+WP[1]*ev[1];
    ev[4]=PA[2]*ev[0]+WP[2]*ev[1];
}

static void ofmo_vrr_cint_psss( const double *ev, double *eh ) {
    int La=1, Lb=0, Lc=0, Ld=0;
    int i, ih, iv;
    // (PS|SS)
    for ( i=0, iv=2, ih=0; i<3; i++, iv++, ih++ ) eh[ih]+=ev[iv];
}

void ofmo_twoint_core_os_psss(
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_vrr_calc_psss(
                    ...
                    ev );
            ofmo_vrr_cint_psss( ev, DINT );
        }	// for (klps)
    }	// for (ijps)
}

int ofmo_twoint_os_psss(
...
int ofmo_twoint_direct_os_psss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int shel_atm[], const int shel_ini[],
        const double atom_x[], const double atom_y[],
