...
static void ofmo_hrr_calc_ddss( double *eh,
...
static void ofmo_xyzint_ddss(
    ...
    // (2,0)
    C10[0] += B10[0];
    xint[  6]=C00[ 0]*xint[  3]+C10[0]*xint[  0];
    yint[  6]=C00[ 1]*yint[  3]+C10[0]*yint[  0];
    zint[  6]=C00[ 2]*zint[  3]+C10[0]*zint[  0];
    C10[1] += B10[1];
    xint[  7]=C00[ 3]*xint[  4]+C10[1]*xint[  1];
    yint[  7]=C00[ 4]*yint[  4]+C10[1]*yint[  1];
    zint[  7]=C00[ 5]*zint[  4]+C10[1]*zint[  1];
    C10[2] += B10[2];
    xint[  8]=C00[ 6]*xint[  5]+C10[2]*xint[  2];
    yint[  8]=C00[ 7]*yint[  5]+C10[2]*yint[  2];
    zint[  8]=C00[ 8]*zint[  5]+C10[2]*zint[  2];
    // (3,0)
    C10[0] += B10[0];
    xint[  9]=C00[ 0]*xint[  6]+C10[0]*xint[  3];
    yint[  9]=C00[ 1]*yint[  6]+C10[0]*yint[  3];
    zint[  9]=C00[ 2]*zint[  6]+C10[0]*zint[  3];
    C10[1] += B10[1];
    xint[ 10]=C00[ 3]*xint[  7]+C10[1]*xint[  4];
    yint[ 10]=C00[ 4]*yint[  7]+C10[1]*yint[  4];
    zint[ 10]=C00[ 5]*zint[  7]+C10[1]*zint[  4];
    C10[2] += B10[2];
    xint[ 11]=C00[ 6]*xint[  8]+C10[2]*xint[  5];
    yint[ 11]=C00[ 7]*yint[  8]+C10[2]*yint[  5];
    zint[ 11]=C00[ 8]*zint[  8]+C10[2]*zint[  5];
    // (4,0)
    C10[0] += B10[0];
    xint[ 12]=C00[ 0]*xint[  9]+C10[0]*xint[  6];
    yint[ 12]=C00[ 1]*yint[  9]+C10[0]*yint[  6];
    zint[ 12]=C00[ 2]*zint[  9]+C10[0]*zint[  6];
    C10[1] += B10[1];
    xint[ 13]=C00[ 3]*xint[ 10]+C10[1]*xint[  7];
    yint[ 13]=C00[ 4]*yint[ 10]+C10[1]*yint[  7];
    zint[ 13]=C00[ 5]*zint[ 10]+C10[1]*zint[  7];
    C10[2] += B10[2];
    xint[ 14]=C00[ 6]*xint[ 11]+C10[2]*xint[  8];
    yint[ 14]=C00[ 7]*yint[ 11]+C10[2]*yint[  8];
    zint[ 14]=C00[ 8]*zint[ 11]+C10[2]*zint[  8];
}

static void ofmo_form_ddss(
        const double *xint, const double *yint, const double *zint,
        double *eh ) {
    // (DS|SS)
    eh[   0] += xint[  6]*yint[  0]*zint[  0];
    eh[   0] += xint[  7]*yint[  1]*zint[  1];
    eh[   0] += xint[  8]*yint[  2]*zint[  2];
    eh[   1] += xint[  0]*yint[  6]*zint[  0];
    eh[   1] += xint[  1]*yint[  7]*zint[  1];
    eh[   1] += xint[  2]*yint[  8]*zint[  2];
    eh[   2] += xint[  0]*yint[  0]*zint[  6];
    eh[   2] += xint[  1]*yint[  1]*zint[  7];
    eh[   2] += xint[  2]*yint[  2]*zint[  8];
    eh[   3] += xint[  3]*yint[  3]*zint[  0];
    eh[   3] += xint[  4]*yint[  4]*zint[  1];
    eh[   3] += xint[  5]*yint[  5]*zint[  2];
    eh[   4] += xint[  3]*yint[  0]*zint[  3];
    eh[   4] += xint[  4]*yint[  1]*zint[  4];
    eh[   4] += xint[  5]*yint[  2]*zint[  5];
    eh[   5] += xint[  0]*yint[  3]*zint[  3];
    eh[   5] += xint[  1]*yint[  4]*zint[  4];
    eh[   5] += xint[  2]*yint[  5]*zint[  5];
    // (FS|SS)
    eh[   6] += xint[  9]*yint[  0]*zint[  0];
    eh[   6] += xint[ 10]*yint[  1]*zint[  1];
    eh[   6] += xint[ 11]*yint[  2]*zint[  2];
    eh[   7] += xint[  0]*yint[  9]*zint[  0];
    eh[   7] += xint[  1]*yint[ 10]*zint[  1];
    eh[   7] += xint[  2]*yint[ 11]*zint[  2];
    eh[   8] += xint[  0]*yint[  0]*zint[  9];
    eh[   8] += xint[  1]*yint[  1]*zint[ 10];
    eh[   8] += xint[  2]*yint[  2]*zint[ 11];
    eh[   9] += xint[  6]*yint[  3]*zint[  0];
    eh[   9] += xint[  7]*yint[  4]*zint[  1];
    eh[   9] += xint[  8]*yint[  5]*zint[  2];
    eh[  10] += xint[  6]*yint[  0]*zint[  3];
    eh[  10] += xint[  7]*yint[  1]*zint[  4];
    eh[  10] += xint[  8]*yint[  2]*zint[  5];
    eh[  11] += xint[  3]*yint[  6]*zint[  0];
    eh[  11] += xint[  4]*yint[  7]*zint[  1];
    eh[  11] += xint[  5]*yint[  8]*zint[  2];
    eh[  12] += xint[  3]*yint[  0]*zint[  6];
    eh[  12] += xint[  4]*yint[  1]*zint[  7];
    eh[  12] += xint[  5]*yint[  2]*zint[  8];
    eh[  13] += xint[  3]*yint[  3]*zint[  3];
    eh[  13] += xint[  4]*yint[  4]*zint[  4];
    eh[  13] += xint[  5]*yint[  5]*zint[  5];
    eh[  14] += xint[  0]*yint[  6]*zint[  3];
    eh[  14] += xint[  1]*yint[  7]*zint[  4];
    eh[  14] += xint[  2]*yint[  8]*zint[  5];
    eh[  15] += xint[  0]*yint[  3]*zint[  6];
    eh[  15] += xint[  1]*yint[  4]*zint[  7];
    eh[  15] += xint[  2]*yint[  5]*zint[  8];
    // (GS|SS)
    eh[  16] += xint[ 12]*yint[  0]*zint[  0];
    eh[  16] += xint[ 13]*yint[  1]*zint[  1];
    eh[  16] += xint[ 14]*yint[  2]*zint[  2];
    eh[  17] += xint[  0]*yint[ 12]*zint[  0];
    eh[  17] += xint[  1]*yint[ 13]*zint[  1];
    eh[  17] += xint[  2]*yint[ 14]*zint[  2];
    eh[  18] += xint[  0]*yint[  0]*zint[ 12];
    eh[  18] += xint[  1]*yint[  1]*zint[ 13];
    eh[  18] += xint[  2]*yint[  2]*zint[ 14];
    eh[  19] += xint[  9]*yint[  3]*zint[  0];
    eh[  19] += xint[ 10]*yint[  4]*zint[  1];
    eh[  19] += xint[ 11]*yint[  5]*zint[  2];
    eh[  20] += xint[  9]*yint[  0]*zint[  3];
    eh[  20] += xint[ 10]*yint[  1]*zint[  4];
    eh[  20] += xint[ 11]*yint[  2]*zint[  5];
    eh[  21] += xint[  6]*yint[  6]*zint[  0];
    eh[  21] += xint[  7]*yint[  7]*zint[  1];
    eh[  21] += xint[  8]*yint[  8]*zint[  2];
    eh[  22] += xint[  6]*yint[  0]*zint[  6];
    eh[  22] += xint[  7]*yint[  1]*zint[  7];
    eh[  22] += xint[  8]*yint[  2]*zint[  8];
    eh[  23] += xint[  6]*yint[  3]*zint[  3];
    eh[  23] += xint[  7]*yint[  4]*zint[  4];
    eh[  23] += xint[  8]*yint[  5]*zint[  5];
    eh[  24] += xint[  3]*yint[  9]*zint[  0];
    eh[  24] += xint[  4]*yint[ 10]*zint[  1];
    eh[  24] += xint[  5]*yint[ 11]*zint[  2];
    eh[  25] += xint[  3]*yint[  0]*zint[  9];
    eh[  25] += xint[  4]*yint[  1]*zint[ 10];
    eh[  25] += xint[  5]*yint[  2]*zint[ 11];
    eh[  26] += xint[  3]*yint[  6]*zint[  3];
    eh[  26] += xint[  4]*yint[  7]*zint[  4];
    eh[  26] += xint[  5]*yint[  8]*zint[  5];
    eh[  27] += xint[  3]*yint[  3]*zint[  6];
    eh[  27] += xint[  4]*yint[  4]*zint[  7];
    eh[  27] += xint[  5]*yint[  5]*zint[  8];
    eh[  28] += xint[  0]*yint[  9]*zint[  3];
    eh[  28] += xint[  1]*yint[ 10]*zint[  4];
    eh[  28] += xint[  2]*yint[ 11]*zint[  5];
    eh[  29] += xint[  0]*yint[  3]*zint[  9];
    eh[  29] += xint[  1]*yint[  4]*zint[ 10];
    eh[  29] += xint[  2]*yint[  5]*zint[ 11];
    eh[  30] += xint[  0]*yint[  6]*zint[  6];
    eh[  30] += xint[  1]*yint[  7]*zint[  7];
    eh[  30] += xint[  2]*yint[  8]*zint[  8];
}

void ofmo_twoint_core_rys_ddss( const int mythread,
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_ddss( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_ddss( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_ddss( eh, BA, DC );
    ofmo_hrr_coef_ddss( eh, DINT );
}

int ofmo_twoint_rys_ddss(
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_ddss( mythread,
...
int ofmo_twoint_direct_rys_ddss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
