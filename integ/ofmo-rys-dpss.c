...
static void ofmo_hrr_clear_dpss( double *eh ) {
    int i;
    // (DS|SS)
...
static void ofmo_hrr_coef_dpss(
...
static void ofmo_hrr_calc_dpss( double *eh,
...
static void ofmo_xyzint_dpss(
    ...
    // (3,0)
    C10[0] += B10[0];
    xint[  6]=C00[ 0]*xint[  4]+C10[0]*xint[  2];
    yint[  6]=C00[ 1]*yint[  4]+C10[0]*yint[  2];
    zint[  6]=C00[ 2]*zint[  4]+C10[0]*zint[  2];
    C10[1] += B10[1];
    xint[  7]=C00[ 3]*xint[  5]+C10[1]*xint[  3];
    yint[  7]=C00[ 4]*yint[  5]+C10[1]*yint[  3];
    zint[  7]=C00[ 5]*zint[  5]+C10[1]*zint[  3];
}

static void ofmo_form_dpss(
        const double *xint, const double *yint, const double *zint,
        double *eh ) {
    // (DS|SS)
    eh[   0] += xint[  4]*yint[  0]*zint[  0];
    eh[   0] += xint[  5]*yint[  1]*zint[  1];
    eh[   1] += xint[  0]*yint[  4]*zint[  0];
    eh[   1] += xint[  1]*yint[  5]*zint[  1];
    eh[   2] += xint[  0]*yint[  0]*zint[  4];
    eh[   2] += xint[  1]*yint[  1]*zint[  5];
    eh[   3] += xint[  2]*yint[  2]*zint[  0];
    eh[   3] += xint[  3]*yint[  3]*zint[  1];
    eh[   4] += xint[  2]*yint[  0]*zint[  2];
    eh[   4] += xint[  3]*yint[  1]*zint[  3];
    eh[   5] += xint[  0]*yint[  2]*zint[  2];
    eh[   5] += xint[  1]*yint[  3]*zint[  3];
    // (FS|SS)
    eh[   6] += xint[  6]*yint[  0]*zint[  0];
    eh[   6] += xint[  7]*yint[  1]*zint[  1];
    eh[   7] += xint[  0]*yint[  6]*zint[  0];
    eh[   7] += xint[  1]*yint[  7]*zint[  1];
    eh[   8] += xint[  0]*yint[  0]*zint[  6];
    eh[   8] += xint[  1]*yint[  1]*zint[  7];
    eh[   9] += xint[  4]*yint[  2]*zint[  0];
    eh[   9] += xint[  5]*yint[  3]*zint[  1];
    eh[  10] += xint[  4]*yint[  0]*zint[  2];
    eh[  10] += xint[  5]*yint[  1]*zint[  3];
    eh[  11] += xint[  2]*yint[  4]*zint[  0];
    eh[  11] += xint[  3]*yint[  5]*zint[  1];
    eh[  12] += xint[  2]*yint[  0]*zint[  4];
    eh[  12] += xint[  3]*yint[  1]*zint[  5];
    eh[  13] += xint[  2]*yint[  2]*zint[  2];
    eh[  13] += xint[  3]*yint[  3]*zint[  3];
    eh[  14] += xint[  0]*yint[  4]*zint[  2];
    eh[  14] += xint[  1]*yint[  5]*zint[  3];
    eh[  15] += xint[  0]*yint[  2]*zint[  4];
    eh[  15] += xint[  1]*yint[  3]*zint[  5];
}

void ofmo_twoint_core_rys_dpss( const int mythread,
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_dpss( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_dpss( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_dpss( eh, BA, DC );
    ofmo_hrr_coef_dpss( eh, DINT );
}

int ofmo_twoint_rys_dpss(
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_dpss( mythread,
...
int ofmo_twoint_direct_rys_dpss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
