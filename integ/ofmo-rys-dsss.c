...
static void ofmo_hrr_clear_dsss( double *eh ) {
    int i;
    // (DS|SS)
    for ( i=0; i<(0+6); i++ ) eh[i] = 0.e0;
}

static void ofmo_hrr_coef_dsss(
...
void ofmo_twoint_core_rys_dsss( const int mythread,
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_dsss( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_dsss( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_dsss( eh, BA, DC );
    ofmo_hrr_coef_dsss( eh, DINT );
}

int ofmo_twoint_rys_dsss(
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_dsss( mythread,
...
int ofmo_twoint_direct_rys_dsss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
