...
static void ofmo_hrr_calc_pppp( double *eh,
...
static void ofmo_form_pppp(
        const double *xint, const double *yint, const double *zint,
        double *eh ) {
    // (PS|PS)
    eh[   0] += xint[ 12]*yint[  0]*zint[  0];
    eh[   0] += xint[ 13]*yint[  1]*zint[  1];
    eh[   0] += xint[ 14]*yint[  2]*zint[  2];
    eh[   1] += xint[  9]*yint[  3]*zint[  0];
    eh[   1] += xint[ 10]*yint[  4]*zint[  1];
    eh[   1] += xint[ 11]*yint[  5]*zint[  2];
    eh[   2] += xint[  9]*yint[  0]*zint[  3];
    eh[   2] += xint[ 10]*yint[  1]*zint[  4];
    eh[   2] += xint[ 11]*yint[  2]*zint[  5];
    eh[   3] += xint[  3]*yint[  9]*zint[  0];
    eh[   3] += xint[  4]*yint[ 10]*zint[  1];
    eh[   3] += xint[  5]*yint[ 11]*zint[  2];
    eh[   4] += xint[  0]*yint[ 12]*zint[  0];
    eh[   4] += xint[  1]*yint[ 13]*zint[  1];
    eh[   4] += xint[  2]*yint[ 14]*zint[  2];
    eh[   5] += xint[  0]*yint[  9]*zint[  3];
    eh[   5] += xint[  1]*yint[ 10]*zint[  4];
    eh[   5] += xint[  2]*yint[ 11]*zint[  5];
    eh[   6] += xint[  3]*yint[  0]*zint[  9];
    eh[   6] += xint[  4]*yint[  1]*zint[ 10];
    eh[   6] += xint[  5]*yint[  2]*zint[ 11];
    eh[   7] += xint[  0]*yint[  3]*zint[  9];
    eh[   7] += xint[  1]*yint[  4]*zint[ 10];
    eh[   7] += xint[  2]*yint[  5]*zint[ 11];
    eh[   8] += xint[  0]*yint[  0]*zint[ 12];
    eh[   8] += xint[  1]*yint[  1]*zint[ 13];
    eh[   8] += xint[  2]*yint[  2]*zint[ 14];
    // (PS|DS)
    eh[   9] += xint[ 15]*yint[  0]*zint[  0];
    eh[   9] += xint[ 16]*yint[  1]*zint[  1];
    eh[   9] += xint[ 17]*yint[  2]*zint[  2];
    eh[  10] += xint[  9]*yint[  6]*zint[  0];
    eh[  10] += xint[ 10]*yint[  7]*zint[  1];
    eh[  10] += xint[ 11]*yint[  8]*zint[  2];
    eh[  11] += xint[  9]*yint[  0]*zint[  6];
    eh[  11] += xint[ 10]*yint[  1]*zint[  7];
    eh[  11] += xint[ 11]*yint[  2]*zint[  8];
    eh[  12] += xint[ 12]*yint[  3]*zint[  0];
    eh[  12] += xint[ 13]*yint[  4]*zint[  1];
    eh[  12] += xint[ 14]*yint[  5]*zint[  2];
    eh[  13] += xint[ 12]*yint[  0]*zint[  3];
    eh[  13] += xint[ 13]*yint[  1]*zint[  4];
    eh[  13] += xint[ 14]*yint[  2]*zint[  5];
    eh[  14] += xint[  9]*yint[  3]*zint[  3];
    eh[  14] += xint[ 10]*yint[  4]*zint[  4];
    eh[  14] += xint[ 11]*yint[  5]*zint[  5];
    eh[  15] += xint[  6]*yint[  9]*zint[  0];
    eh[  15] += xint[  7]*yint[ 10]*zint[  1];
    eh[  15] += xint[  8]*yint[ 11]*zint[  2];
    eh[  16] += xint[  0]*yint[ 15]*zint[  0];
    eh[  16] += xint[  1]*yint[ 16]*zint[  1];
    eh[  16] += xint[  2]*yint[ 17]*zint[  2];
    eh[  17] += xint[  0]*yint[  9]*zint[  6];
    eh[  17] += xint[  1]*yint[ 10]*zint[  7];
    eh[  17] += xint[  2]*yint[ 11]*zint[  8];
    eh[  18] += xint[  3]*yint[ 12]*zint[  0];
    eh[  18] += xint[  4]*yint[ 13]*zint[  1];
    eh[  18] += xint[  5]*yint[ 14]*zint[  2];
    eh[  19] += xint[  3]*yint[  9]*zint[  3];
    eh[  19] += xint[  4]*yint[ 10]*zint[  4];
    eh[  19] += xint[  5]*yint[ 11]*zint[  5];
    eh[  20] += xint[  0]*yint[ 12]*zint[  3];
    eh[  20] += xint[  1]*yint[ 13]*zint[  4];
    eh[  20] += xint[  2]*yint[ 14]*zint[  5];
    eh[  21] += xint[  6]*yint[  0]*zint[  9];
    eh[  21] += xint[  7]*yint[  1]*zint[ 10];
    eh[  21] += xint[  8]*yint[  2]*zint[ 11];
    eh[  22] += xint[  0]*yint[  6]*zint[  9];
    eh[  22] += xint[  1]*yint[  7]*zint[ 10];
    eh[  22] += xint[  2]*yint[  8]*zint[ 11];
    eh[  23] += xint[  0]*yint[  0]*zint[ 15];
    eh[  23] += xint[  1]*yint[  1]*zint[ 16];
    eh[  23] += xint[  2]*yint[  2]*zint[ 17];
    eh[  24] += xint[  3]*yint[  3]*zint[  9];
    eh[  24] += xint[  4]*yint[  4]*zint[ 10];
    eh[  24] += xint[  5]*yint[  5]*zint[ 11];
    eh[  25] += xint[  3]*yint[  0]*zint[ 12];
    eh[  25] += xint[  4]*yint[  1]*zint[ 13];
    eh[  25] += xint[  5]*yint[  2]*zint[ 14];
    eh[  26] += xint[  0]*yint[  3]*zint[ 12];
    eh[  26] += xint[  1]*yint[  4]*zint[ 13];
    eh[  26] += xint[  2]*yint[  5]*zint[ 14];
    // (DS|PS)
    eh[  27] += xint[ 21]*yint[  0]*zint[  0];
    eh[  27] += xint[ 22]*yint[  1]*zint[  1];
    eh[  27] += xint[ 23]*yint[  2]*zint[  2];
    eh[  28] += xint[ 18]*yint[  3]*zint[  0];
    eh[  28] += xint[ 19]*yint[  4]*zint[  1];
    eh[  28] += xint[ 20]*yint[  5]*zint[  2];
    eh[  29] += xint[ 18]*yint[  0]*zint[  3];
    eh[  29] += xint[ 19]*yint[  1]*zint[  4];
    eh[  29] += xint[ 20]*yint[  2]*zint[  5];
    eh[  30] += xint[  3]*yint[ 18]*zint[  0];
    eh[  30] += xint[  4]*yint[ 19]*zint[  1];
    eh[  30] += xint[  5]*yint[ 20]*zint[  2];
    eh[  31] += xint[  0]*yint[ 21]*zint[  0];
    eh[  31] += xint[  1]*yint[ 22]*zint[  1];
    eh[  31] += xint[  2]*yint[ 23]*zint[  2];
    eh[  32] += xint[  0]*yint[ 18]*zint[  3];
    eh[  32] += xint[  1]*yint[ 19]*zint[  4];
    eh[  32] += xint[  2]*yint[ 20]*zint[  5];
    eh[  33] += xint[  3]*yint[  0]*zint[ 18];
    eh[  33] += xint[  4]*yint[  1]*zint[ 19];
    eh[  33] += xint[  5]*yint[  2]*zint[ 20];
    eh[  34] += xint[  0]*yint[  3]*zint[ 18];
    eh[  34] += xint[  1]*yint[  4]*zint[ 19];
    eh[  34] += xint[  2]*yint[  5]*zint[ 20];
    eh[  35] += xint[  0]*yint[  0]*zint[ 21];
    eh[  35] += xint[  1]*yint[  1]*zint[ 22];
    eh[  35] += xint[  2]*yint[  2]*zint[ 23];
    eh[  36] += xint[ 12]*yint[  9]*zint[  0];
    eh[  36] += xint[ 13]*yint[ 10]*zint[  1];
    eh[  36] += xint[ 14]*yint[ 11]*zint[  2];
    eh[  37] += xint[  9]*yint[ 12]*zint[  0];
    eh[  37] += xint[ 10]*yint[ 13]*zint[  1];
    eh[  37] += xint[ 11]*yint[ 14]*zint[  2];
    eh[  38] += xint[  9]*yint[  9]*zint[  3];
    eh[  38] += xint[ 10]*yint[ 10]*zint[  4];
    eh[  38] += xint[ 11]*yint[ 11]*zint[  5];
    eh[  39] += xint[ 12]*yint[  0]*zint[  9];
    eh[  39] += xint[ 13]*yint[  1]*zint[ 10];
    eh[  39] += xint[ 14]*yint[  2]*zint[ 11];
    eh[  40] += xint[  9]*yint[  3]*zint[  9];
    eh[  40] += xint[ 10]*yint[  4]*zint[ 10];
    eh[  40] += xint[ 11]*yint[  5]*zint[ 11];
    eh[  41] += xint[  9]*yint[  0]*zint[ 12];
    eh[  41] += xint[ 10]*yint[  1]*zint[ 13];
    eh[  41] += xint[ 11]*yint[  2]*zint[ 14];
    eh[  42] += xint[  3]*yint[  9]*zint[  9];
    eh[  42] += xint[  4]*yint[ 10]*zint[ 10];
    eh[  42] += xint[  5]*yint[ 11]*zint[ 11];
    eh[  43] += xint[  0]*yint[ 12]*zint[  9];
    eh[  43] += xint[  1]*yint[ 13]*zint[ 10];
    eh[  43] += xint[  2]*yint[ 14]*zint[ 11];
    eh[  44] += xint[  0]*yint[  9]*zint[ 12];
    eh[  44] += xint[  1]*yint[ 10]*zint[ 13];
    eh[  44] += xint[  2]*yint[ 11]*zint[ 14];
    // (DS|DS)
    eh[  45] += xint[ 24]*yint[  0]*zint[  0];
    eh[  45] += xint[ 25]*yint[  1]*zint[  1];
    eh[  45] += xint[ 26]*yint[  2]*zint[  2];
    eh[  46] += xint[ 18]*yint[  6]*zint[  0];
    eh[  46] += xint[ 19]*yint[  7]*zint[  1];
    eh[  46] += xint[ 20]*yint[  8]*zint[  2];
    eh[  47] += xint[ 18]*yint[  0]*zint[  6];
    eh[  47] += xint[ 19]*yint[  1]*zint[  7];
    eh[  47] += xint[ 20]*yint[  2]*zint[  8];
    eh[  48] += xint[ 21]*yint[  3]*zint[  0];
    eh[  48] += xint[ 22]*yint[  4]*zint[  1];
    eh[  48] += xint[ 23]*yint[  5]*zint[  2];
    eh[  49] += xint[ 21]*yint[  0]*zint[  3];
    eh[  49] += xint[ 22]*yint[  1]*zint[  4];
    eh[  49] += xint[ 23]*yint[  2]*zint[  5];
    eh[  50] += xint[ 18]*yint[  3]*zint[  3];
    eh[  50] += xint[ 19]*yint[  4]*zint[  4];
    eh[  50] += xint[ 20]*yint[  5]*zint[  5];
    eh[  51] += xint[  6]*yint[ 18]*zint[  0];
    eh[  51] += xint[  7]*yint[ 19]*zint[  1];
    eh[  51] += xint[  8]*yint[ 20]*zint[  2];
    eh[  52] += xint[  0]*yint[ 24]*zint[  0];
    eh[  52] += xint[  1]*yint[ 25]*zint[  1];
    eh[  52] += xint[  2]*yint[ 26]*zint[  2];
    eh[  53] += xint[  0]*yint[ 18]*zint[  6];
    eh[  53] += xint[  1]*yint[ 19]*zint[  7];
    eh[  53] += xint[  2]*yint[ 20]*zint[  8];
    eh[  54] += xint[  3]*yint[ 21]*zint[  0];
    eh[  54] += xint[  4]*yint[ 22]*zint[  1];
    eh[  54] += xint[  5]*yint[ 23]*zint[  2];
    eh[  55] += xint[  3]*yint[ 18]*zint[  3];
    eh[  55] += xint[  4]*yint[ 19]*zint[  4];
    eh[  55] += xint[  5]*yint[ 20]*zint[  5];
    eh[  56] += xint[  0]*yint[ 21]*zint[  3];
    eh[  56] += xint[  1]*yint[ 22]*zint[  4];
    eh[  56] += xint[  2]*yint[ 23]*zint[  5];
    eh[  57] += xint[  6]*yint[  0]*zint[ 18];
    eh[  57] += xint[  7]*yint[  1]*zint[ 19];
    eh[  57] += xint[  8]*yint[  2]*zint[ 20];
    eh[  58] += xint[  0]*yint[  6]*zint[ 18];
    eh[  58] += xint[  1]*yint[  7]*zint[ 19];
    eh[  58] += xint[  2]*yint[  8]*zint[ 20];
    eh[  59] += xint[  0]*yint[  0]*zint[ 24];
    eh[  59] += xint[  1]*yint[  1]*zint[ 25];
    eh[  59] += xint[  2]*yint[  2]*zint[ 26];
    eh[  60] += xint[  3]*yint[  3]*zint[ 18];
    eh[  60] += xint[  4]*yint[  4]*zint[ 19];
    eh[  60] += xint[  5]*yint[  5]*zint[ 20];
    eh[  61] += xint[  3]*yint[  0]*zint[ 21];
    eh[  61] += xint[  4]*yint[  1]*zint[ 22];
    eh[  61] += xint[  5]*yint[  2]*zint[ 23];
    eh[  62] += xint[  0]*yint[  3]*zint[ 21];
    eh[  62] += xint[  1]*yint[  4]*zint[ 22];
    eh[  62] += xint[  2]*yint[  5]*zint[ 23];
    eh[  63] += xint[ 15]*yint[  9]*zint[  0];
    eh[  63] += xint[ 16]*yint[ 10]*zint[  1];
    eh[  63] += xint[ 17]*yint[ 11]*zint[  2];
    eh[  64] += xint[  9]*yint[ 15]*zint[  0];
    eh[  64] += xint[ 10]*yint[ 16]*zint[  1];
    eh[  64] += xint[ 11]*yint[ 17]*zint[  2];
    eh[  65] += xint[  9]*yint[  9]*zint[  6];
    eh[  65] += xint[ 10]*yint[ 10]*zint[  7];
    eh[  65] += xint[ 11]*yint[ 11]*zint[  8];
    eh[  66] += xint[ 12]*yint[ 12]*zint[  0];
    eh[  66] += xint[ 13]*yint[ 13]*zint[  1];
    eh[  66] += xint[ 14]*yint[ 14]*zint[  2];
    eh[  67] += xint[ 12]*yint[  9]*zint[  3];
    eh[  67] += xint[ 13]*yint[ 10]*zint[  4];
    eh[  67] += xint[ 14]*yint[ 11]*zint[  5];
    eh[  68] += xint[  9]*yint[ 12]*zint[  3];
    eh[  68] += xint[ 10]*yint[ 13]*zint[  4];
    eh[  68] += xint[ 11]*yint[ 14]*zint[  5];
    eh[  69] += xint[ 15]*yint[  0]*zint[  9];
    eh[  69] += xint[ 16]*yint[  1]*zint[ 10];
    eh[  69] += xint[ 17]*yint[  2]*zint[ 11];
    eh[  70] += xint[  9]*yint[  6]*zint[  9];
    eh[  70] += xint[ 10]*yint[  7]*zint[ 10];
    eh[  70] += xint[ 11]*yint[  8]*zint[ 11];
    eh[  71] += xint[  9]*yint[  0]*zint[ 15];
    eh[  71] += xint[ 10]*yint[  1]*zint[ 16];
    eh[  71] += xint[ 11]*yint[  2]*zint[ 17];
    eh[  72] += xint[ 12]*yint[  3]*zint[  9];
    eh[  72] += xint[ 13]*yint[  4]*zint[ 10];
    eh[  72] += xint[ 14]*yint[  5]*zint[ 11];
    eh[  73] += xint[ 12]*yint[  0]*zint[ 12];
    eh[  73] += xint[ 13]*yint[  1]*zint[ 13];
    eh[  73] += xint[ 14]*yint[  2]*zint[ 14];
    eh[  74] += xint[  9]*yint[  3]*zint[ 12];
    eh[  74] += xint[ 10]*yint[  4]*zint[ 13];
    eh[  74] += xint[ 11]*yint[  5]*zint[ 14];
    eh[  75] += xint[  6]*yint[  9]*zint[  9];
    eh[  75] += xint[  7]*yint[ 10]*zint[ 10];
    eh[  75] += xint[  8]*yint[ 11]*zint[ 11];
    eh[  76] += xint[  0]*yint[ 15]*zint[  9];
    eh[  76] += xint[  1]*yint[ 16]*zint[ 10];
    eh[  76] += xint[  2]*yint[ 17]*zint[ 11];
    eh[  77] += xint[  0]*yint[  9]*zint[ 15];
    eh[  77] += xint[  1]*yint[ 10]*zint[ 16];
    eh[  77] += xint[  2]*yint[ 11]*zint[ 17];
    eh[  78] += xint[  3]*yint[ 12]*zint[  9];
    eh[  78] += xint[  4]*yint[ 13]*zint[ 10];
    eh[  78] += xint[  5]*yint[ 14]*zint[ 11];
    eh[  79] += xint[  3]*yint[  9]*zint[ 12];
    eh[  79] += xint[  4]*yint[ 10]*zint[ 13];
    eh[  79] += xint[  5]*yint[ 11]*zint[ 14];
    eh[  80] += xint[  0]*yint[ 12]*zint[ 12];
    eh[  80] += xint[  1]*yint[ 13]*zint[ 13];
    eh[  80] += xint[  2]*yint[ 14]*zint[ 14];
}

void ofmo_twoint_core_rys_pppp( const int mythread,
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_pppp( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_pppp( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_pppp( eh, BA, DC );
    ofmo_hrr_coef_pppp( eh, DINT );
}

int ofmo_twoint_rys_pppp(
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_pppp( mythread,
...
int ofmo_twoint_direct_rys_pppp(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
