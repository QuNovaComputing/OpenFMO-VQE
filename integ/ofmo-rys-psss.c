...
static void ofmo_hrr_clear_psss( double *eh ) {
    int i;
    // (PS|SS)
    for ( i=0; i<(0+3); i++ ) eh[i] = 0.e0;
}

static void ofmo_hrr_coef_psss(
...
static void ofmo_form_psss(
        const double *xint, const double *yint, const double *zint,
        double *eh ) {
    // (PS|SS)
    eh[   0] += xint[  1]*yint[  0]*zint[  0];
    eh[   1] += xint[  0]*yint[  1]*zint[  0];
    eh[   2] += xint[  0]*yint[  0]*zint[  1];
}

void ofmo_twoint_core_rys_psss( const int mythread,
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_psss( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_psss( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_psss( eh, BA, DC );
    ofmo_hrr_coef_psss( eh, DINT );
}

int ofmo_twoint_rys_psss(
    ...
    max_nzeri = ebuf_max_nzeri - 3*1*1*1;
    nzeri4    = nzeri*4;
    if ( nzeri >= max_nzeri ) {
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_psss( mythread,
...
int ofmo_twoint_direct_rys_psss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
