...
static void ofmo_hrr_clear_ssss( double *eh ) {
    int i;
    // (SS|SS)
    for ( i=0; i<(0+1); i++ ) eh[i] = 0.e0;
}

static void ofmo_hrr_coef_ssss(
...
static void ofmo_xyzint_ssss(
    ...
    double C10[1], CP10[1], CP01[1], C01[1];
    // (0,0)
    xint[  0]=1.e0;
    yint[  0]=1.e0;
    zint[  0]=F00[0];
...
static void ofmo_form_ssss(
        const double *xint, const double *yint, const double *zint,
        double *eh ) {
    // (SS|SS)
    eh[   0] += xint[  0]*yint[  0]*zint[  0];
}

void ofmo_twoint_core_rys_ssss( const int mythread,
    ...
    double C00[3], CP00[3], B00[1], B10[1], B01[1], F00[1];
    ...
    for ( ijps=0; ijps<(*nijps); ijps++ ) {
        ...
        for ( klps=0; klps<(*nklps); klps++ ) {
            ...
            ofmo_xyzint_ssss( F00, B00, B10, B01, C00, CP00,
                     xint, yint, zint );
            ofmo_form_ssss( xint, yint, zint, eh );
        }
    }
    ofmo_hrr_calc_ssss( eh, BA, DC );
    ofmo_hrr_coef_ssss( eh, DINT );
}

int ofmo_twoint_rys_ssss(
    ...
    max_nzeri = ebuf_max_nzeri - 1*1*1*1;
    nzeri4    = nzeri*4;
    if ( nzeri >= max_nzeri ) {
    ...
    for ( ijcs=ijcs0+workerid; ijcs<ijcs1; ijcs+=nworkers ) {
        ...
        for ( klcs=klcs0; klcs<max_klcs; klcs++ ) {
            ...
            ofmo_twoint_core_rys_ssss( mythread,
            ...
            for ( i=0, iao=iao0, ix=0; i<1; i++, iao++ ) {
                I2 = (iao*iao+iao)>>1;
...
int ofmo_twoint_direct_rys_ssss(
        const int *pnworkers, const int *pworkerid,
        const int *pLa, const int *pLb, const int *pLc, const int *pLd,
        const int *shel_atm, const int *shel_ini,
        const double atom_x[], const double atom_y[],
