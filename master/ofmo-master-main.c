...
//    printf("port name prefix      = %s\n", port_name_prefix );
    ...
    fflush(stdout);
    etscc0 = MPI_Wtime();
    for ( itera=1; itera<=maxscc; itera++ ) {               // SCC start here
	et0 = MPI_Wtime();
    ...
    char* version = NULL;
    ofmo_data_get_vals("version", &version);
	for ( int i=0; i<nfrag; i++ ) {
	    ...
	    req_workers[workerid] = ofmo_call_async_worker(                 //worker started
		    comm_workers[workerid],
    ...
    //  Start run VQE on monomer now
    char* version = NULL;
    ofmo_data_get_vals("version", &version);
    if (strcmp(version, "2") == 0)
    {
        printf("##VERSION 2: VQE monomer iteration\n");
        fflush(stdout);
        total_energy = total_energy0 = 0.e0;
        imsg[OFMO_I_SCC] = -1;
        for ( int i=0; i<nfrag; i++ ) {
	...
	printf(" energy = %16.8f  energy0 = %16.8f  ( %10.6f )\n",
		total_energy, total_energy0, (et1-et0) );
    printf("## VQE monomer iteration complete\n");
	fflush( stdout );
	// Exchange (update) monomer data
	ofmo_worker_barrier( nworkers, comm_workers );
	//ofmo_mserv_barrier( nmservs, comm_mservs );
		...
		if ( dist[jfrag] < ldim ) {
            ...
            if(!imsg_method_def_flag){
                if(method == OFMO_UNDEF){
                    ...
                    else if(im == OFMO_RHF && jm == OFMO_VQE) imsg[OFMO_I_METHOD] = OFMO_RHF;
                    else if(im == OFMO_VQE && jm == OFMO_RHF) imsg[OFMO_I_METHOD] = OFMO_RHF;
                    else{
    ...
    //aidan
    printf("    total_energy0(FMO) = %16.8f\n", total_energy0 );
    printf("    de0scf = %16.8f\n", de0scf );
    printf("    total_esdimer0 = %16.8f\n", total_esdimer0 );
    //aidan done
